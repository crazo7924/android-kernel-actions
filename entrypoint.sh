...
if [[ $arch = "arm64" ]]; then
    ...
    if [[ $compiler = gcc/* ]]; then
        ...
        url="https://github.com/LineageOS/android_prebuilts_gcc_linux-x86_aarch64_aarch64-linux-android-4.9/archive/lineage-18.1.tar.gz"
        echo "Downloading $url"
        if ! wget --no-check-certificate "$url" -O /tmp/aosp-gcc-arm64.tar.gz &>/dev/null; then
            err "Failed downloading toolchain, refer to the README for details"
            exit 1
        fi
        url="https://github.com/LineageOS/android_prebuilts_gcc_linux-x86_arm_arm-linux-androideabi-4.9/archive/lineage-18.1.tar.gz"
        echo "Downloading $url"
...
if ! make O=out $arch_opts $make_opts $host_make_opts -j"$(nproc --all) all"; then
    err "Failed building kernel and modules, probably the toolchain is not compatible with the kernel, or kernel source problem"
    exit 3
fi
